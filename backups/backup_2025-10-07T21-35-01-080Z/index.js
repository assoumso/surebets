...
async function analyze(dateStr = new Date().toISOString().split('T')[0]) {
  // In analyze function, modify the switch to handle arbitrary past dates by using cache only
  ...
  switch (diffDays) {
    ...
    default:
      if (diffDays < -1) {
        ...
        throw new Error('Date non supportée');
      }
  }
  if (dateParam !== null) {
    url = urlMap[dateParam];
  }
  console.log(`Analyse des matchs pour ${dateParam || 'past date'} (${dateStr})`);
  ...
  if (!url) {
    console.log(`No URL for past date ${dateStr}, and no cache available. Returning empty results.`);
    return [];
  }
